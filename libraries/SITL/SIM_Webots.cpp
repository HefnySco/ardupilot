...
/*
  ...
  inside some other key or value that merely contains those letters, and
  before end if that is given
 */
const char *Webots::find_key(const char *p, const char *key, const char *end)
{
    const size_t len = strlen(key);
    const char *start = p;
    while ((p = strstr(p, key)) != nullptr) {
        if (end != nullptr && p + len >= end) {
            // past the end of the section: a required key must not be taken
            // from whichever section happens to follow
            return nullptr;
        }
...
bool Webots::parse_sensors(const char *json)
{
    ...
    for (uint16_t i=0; i<ARRAY_SIZE(keytable); i++) {
        ...
        // a named section's keys are looked for only between its braces
        // (sections hold no nested objects); top-level keys anywhere
        const char *end = nullptr;
        if (key.section[0] != 0) {
            const char *open = strchr(p, '{');
            end = (open != nullptr) ? strchr(open, '}') : nullptr;
            if (end == nullptr) {
                parse_error("section %s is not an object", key.section);
                return false;
            }
        }

        // find key inside section
        p = find_key(p, key.key, end);
        if (!p) {
            if (!key.required) {
...
void Webots::update(const struct sitl_input &input)
{
    ...
    // RPM instance, and ESC telemetry reports a motor for each, so set only
    // the channels that drive a rotor.
    motor_mask = (state.rpm_count >= 32) ? 0xFFFFFFFFU : ((1U << state.rpm_count) - 1U);
    if (output_type == OUTPUT_TRICOPTER) {
        // rotors on SERVO1, SERVO2 and SERVO4 (output_tricopter()); the "rpm"
        // entry for SERVO3 is only a placeholder
        motor_mask &= ~(1U << 2);
    }
