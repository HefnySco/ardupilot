...
void getInertia (const WbDeviceTag inertialUnit, const double coordinateSystem_NUE, char *buf)
{
  ...
  {
    sprintf(buf,"\"Rroll\": %f,\"pitch\": %f,\"yaw\": %f",inertial_directions[0], -inertial_directions[1], inertial_directions[2]);
  }
...
void getGPS (const WbDeviceTag gps, const double coordinateSystem_NUE, char *buf)
{
    ...
    {
      sprintf(buf,"\"Xx\": %f,\"y\": %f,\"z\": %f", north3D[2], -north3D[0], north3D[1]);
    }
...
*/
        ...
        getGyro(gyro, coordinateSystem_NUE, gyro_buf);
        getAcc(accelerometer, coordinateSystem_NUE, acc_buf);
        getCompass(compass, coordinateSystem_NUE, compass_buf);
        getGPS(gps, coordinateSystem_NUE, gps_buf);
        getInertia (inertial_unit, coordinateSystem_NUE, inertial_buf);
        getLinearVelocity(self_node, coordinateSystem_NUE, linear_velocity_buf);
